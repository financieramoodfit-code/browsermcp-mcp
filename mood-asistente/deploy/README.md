...
> `mood-asistente/run-round.sh`.

> ⚠️ **Antes de dejarlo solo:** en modo desatendido el asistente **envía mensajes
> reales a tus clientes sin que nadie los revise**. Mirá las primeras rondas a
> mano (logs en `mood-asistente/logs/`) y recién después instalá el servicio.
...
```powershell
cd mood-asistente\deploy\windows
...
- Aparecen logs en `mood-asistente/logs/ronda-*.log`.
